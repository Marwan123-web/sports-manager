...
import { teamsApi, playersApi, getErrorMessage } from "@/lib/api";
...
export default function TeamDetail() {
  ...
  useEffect(() => {
    setNewPlayer({ ...newPlayer, position: positions[0] });
  }, [positions]);
  ...
  const fetchTeam = async () => {
    try {
      setLoading(true);
      setError("");
      const data = await teamsApi.getTeam(params.teamId as string, q);
      setTeam(data);
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex justify-between items-start">
        ...
        {user?.id == team.tournament.creator.id &&
      ...
      {/* Players Table */}
      <section>
        <div className="flex flex-row gap-4 items-center mb-6 justify-between">
      ...
      >
        ...
        <Select
          ...
          id="position"
          value={newPlayer.position ?? positions[0]}
          onChange={(e) =>
